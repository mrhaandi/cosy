...
if __name__ == "__main__":
    ...
    target = Constructor("AVL", Constructor("height", Literal(5)) & Constructor("label", Literal(None)))
    ...
    start_time = time.time()
    terms = itertools.islice(solution_space.enumerate_trees_lazy(target), 200)
    end_time = time.time()
...
if False:#__name__ == "__main__":
    ...
    start_time = time.time()
    terms = itertools.islice(solution_space.enumerate_trees_lazy(target), 20)
    end_time = time.time()
